...
public class BuildBuilding : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void StartBuilding()
    {
        ...
        if (paused == false)
        {
            ...
            //if (done == true && Camera.main.GetComponent<CameraControl>().draggedBuilding == null)
            if (done == true && !GameObject.Find("Builder2(Clone)") && manageGame.buildingsInProgress)
            {
                //GameObject builtBuilding = Instantiate(model,
                ...
                //ActiveBuilder = Instantiate(Builder, new Vector3(0, -5, 0), Quaternion.identity);
                ActiveBuilder = Instantiate(Builder, new Vector3(0, -5, 0), model.transform.rotation);
                //ActiveBuilder = Instantiate(Builder, new Vector3(0, -5, 0), Quaternion.Euler(0f, -90f, 0f));
                manageGame.buildingsInProgress = false;
                manageGame.buildingsInProgressSound = false;
                SetButtonsInteractable(true);
            }
            //else if (inProgress == false && Camera.main.GetComponent<CameraControl>().draggedBuilding == null)
