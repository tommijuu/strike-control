...
public class UnitController : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        //angularSpeed = myAgent.angularSpeed;
    ...
    {
        // if(AgentStopped() && currentState == State.Idle)
        // {
    ...
    public void Shoot()
    {
        ...
        //Audiomanager.instance.PlayCombatSound(gameObject.name, 1f);
    ...
    {
        if(stats.name == "C_LightTankStats" || stats.name == "E_LightTankStats" || stats.name == "U_LightTankStats")
        {
            //transform.position = Vector3.MoveTowards(transform.position, targetEnemy.transform.position, 0.2f);
        }
            ...
            while (targetEnemy != null && targetHealth.health > 0)
            {
                if (IsTargetInRange())
                {
                    ...
                    if (currentState == State.Moving)
                        yield break;
                    /*
                    if (!LooksAtTarget() && !gameObject.name.Equals("TurretModel(Clone)"))
                    {
                        //if(!rotatingTowardsTarget)
                            //StartCoroutine(RotateTowardsTarget());
                    }
                    else if(!rotatingTowardsTarget || targetEnemy.name.Equals("TurretModel(Clone)"))
                    {
                    */
